...
function verify (req, tokenset, next) {
  ...
  try {
    ...
    const memberIDs = new Set(User.find(data).filter(m => m.exact).map(m => m.id))
    if (memberIDs.size > 1) {
      // would like to handle this situation at some point
      throw new Error(`${memberIDs.size} existing Users match this ORCiD`)
    }
    ...
    if (req.user instanceof User) { // User already signed in · Linking/Merging
      if (account.user) { // ORCiD account already associated with a user
        if (memberIDs.has(req.user.id) && memberIDs.has(account.user)) { // implies req.user.id ≡ account.user
      ...
      let memberID = account.user // match based on ORCiD
      if (!memberID && memberIDs.size > 0) memberID = Array.from(memberIDs)[0]
      if (memberID) {
