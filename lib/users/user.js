...
export class User {
  ...
  static membersByRole (roleName) {
    return data.roles.users(roleName).map(id => ({ id, name: data.names.get(id) })).sort((a, b) => a.name.sort.localeCompare(b.name.sort)).map(n => ({ user: n.id, name: n.name.full }))
  }

  static search (query, membersOnly = false) {
    const terms = query.toLowerCase().split(/\W+/)
    const hits = new Set()
    const members = new Set()

    if (membersOnly) {
      for (const id of data.roles.users('User')) {
        members.add(id)
      }
    }
    ...
    for (const email of data.emails.all()) {
      ...
      if (membersOnly && !members.has(email.user)) continue
    ...
    for (const [user, name] of names) {
      ...
      if (membersOnly && !members.has(user)) continue
  ...
  static mayEdit (user, targetID) {
    ...
    if (user instanceof User && user.hasRole(config.users.role)) return true
    return false
  }
