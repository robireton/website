...
menu.add([
  ['/account', 'Account', { weight: 99, roles: ['authenticated'] }],
  ['/users', 'Users', { roles: ['Administrator'] }],
  ['/login', 'Login', { weight: 100, roles: ['unauthenticated'], hideself: true }],
  ['/logout', 'Logout', { weight: 100, roles: ['authenticated'], hideself: true }]
])
...
router.get('/merge', (req, res) => {
  ...
  try {
    ...
    if (target && source) {
      ...
      req.session.userSearchID = user.id
  ...
  try {
    ...
    if (req.user instanceof User) {
      ...
      const page = {
        ...
        stylesheets: ['users.css'],
        admin: req.user.hasRole('Administrator'),
        profile,
        actions: {
...
router.get('/profile', (req, res) => {
  ...
  try {
    ...
    let user = req.user || {}
    if (user instanceof User && user.hasRole('Administrator') && req.query.id) {
...
router.post('/profile', async (req, res) => {
  ...
  try {
    ...
    journal.write('Users', `${req.method} ${req.originalUrl}`, err.message, err)
    res.status(500).render('error', {
      pageid: 'profile-update-error-page',
      page_title: 'Error',
      base_url: req.proxyBase,
      sitename: config.sitename,
      sitemenu: menu.entries(req.path, req.proxyBase, req.user),
...
router.get('/users/search', (req, res) => {
  ...
  try {
    if (!(req.user instanceof User && req.user.hasRole('Administrator'))) {
      res.status(403).render('markup', {
        pageid: '403-error-page',
    ...
    if (req.session.userSearchID) {
      try {
        const text = User.getDisplayEmail(req.session.userSearchID)
        if (text) context.initial = { value: req.session.userSearchID, text }
      } catch (_) {
        delete req.session.userSearchID
      }
    }
...
router.get('/users/members', (req, res) => {
  if (!(req.user instanceof User && req.user.hasRole('Administrator'))) return res.sendStatus(403)
  ...
  if ('id' in req.query) {
    try {
      req.session.userSearchID = req.query.id
